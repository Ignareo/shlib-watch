...
- **分组**：以 `## 组名` 开头的行是分组标题，作用于其后所有书，直到下一个组头；单独的 `##` 表示回到未分组。看板按组分节展示书卡，便于书单变长后按主题浏览，例如：
  ...
  中国建筑史 | TU-092/3965-20; TU-092/3965-17; TU-092/3965-19
  ```
- **标签**：第四列可写一个或多个标签（逗号/全角逗号分隔）。注意：标签目前只被采样器解析存入 index.json 存档，**看板不展示、书单助手不编辑**（前端已移除标签 UI）
- 书号（book id）与历史数据按索书号集合自动关联：增删书、调整行序、合并/拆分行都不会串历史
- 看板底部的「书单助手」支持录入/导入 txt、分段式分组编辑（组名重命名/合并/解散、拖拽归组调序）、索书号随改随校验、一键下载标准 books.txt
...
- **筛选**：可按「全部 / 可以外借 / 全部借出 / 仅馆内阅览」过滤书卡；书单有分组时还会出现「分组」chips（单选，再点取消），书卡按分组小节展示，两种筛选可叠加
- **分享**：每张书卡标题旁有「分享」按钮——手机上调起系统分享，电脑上复制带 `#card-id` 锚点的链接，接收者打开即定位到该书；页面带 Open Graph 分享卡片（`docs/og-image.png`，部署后建议把 og:image 改为绝对 URL）
